...
// ─── Admin Login Page ─────────────────────────────────────────────────────────
function AdminLogin({ onLogin, alertMsg, navigateTo }) {
    ...
    <div className="main-container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '80vh' }}>
      ...
      <div style={{ width: '100%', maxWidth: '460px' }}>
        ...
        <div style={{
          ...
          <form onSubmit={handleSubmit} onKeyDown={handleEnterKeyNavigation}>
            ...
            <div className="form-group" style={{ position: 'relative' }}>
              <input
                ...
                onKeyDown={e => setCapsLockActive(e.getModifierState && e.getModifierState('CapsLock'))}
                onKeyUp={e => setCapsLockActive(e.getModifierState && e.getModifierState('CapsLock'))}
                required
            ...
            {capsLockActive && (
              <div className="caps-lock-badge" style={{ marginBottom: '1rem' }}>
                ⇪ Caps Lock is ON
              </div>
            )}
            <button
          ...
          <div style={{ textAlign: 'center', marginTop: '1.5rem' }}>
            <a href="#" onClick={(e) => { e.preventDefault(); if (navigateTo) navigateTo('voter'); else window.location.hash = ''; }} style={{ color: 'var(--text-muted)', fontSize: '0.82rem', textDecoration: 'none', fontWeight: 600 }}>
              ← Go to Voter Portal
            </a>
          </div>
        </div>
...
// ─── Main Admin Console ───────────────────────────────────────────────────────
export default function AdminConsole({ adminUser, setAdminUser, navigateTo }) {
  ...
  // ─── Effects ─────────────────────────────────────────────────────────────
  useEffect(() => {
    // Only restore session if present in sessionStorage (preserves active session on same page reload)
    const saved = sessionStorage.getItem('votepulse_admin');
    if (saved) {
      try { setAdminUser(JSON.parse(saved)); } catch (e) {}
    }
    return () => {
      // Auto-logout when leaving the Admin module page
      setAdminUser(null);
  ...
  const handleAdminLogin = (id, pass) => {
    if (pass === 'admin123' || pass === 'voter123') {
      ...
      sessionStorage.setItem('votepulse_admin', JSON.stringify(u));
      setAdminUser(u);
    } else {
  ...
  const fetchAdminStats = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.success) {
          ...
          setVoters((data.voters || []).filter(v => v.role !== 'admin' && !(v.voter_id || '').toUpperCase().startsWith('ADM-')));
    ...
    const votersArr = JSON.parse(localStorage.getItem('votepulse_admin_voters') || '[]')
      .filter(v => v.role !== 'admin' && !(v.voter_id || '').toUpperCase().startsWith('ADM-'));
  ...
  const deleteElection = async (id) => {
    try {
      ...
      if (res.ok && data.success) {
        showAlert('Election and associated candidates/ballots deleted permanently.', 'success');
      } else {
        showAlert(data.message || 'Election deleted.', 'info');
      }
    } catch (err) {
      console.warn('Election delete request notice:', err);
    }
  ...
  // ─── Computed values for Results tab & Automatic Winner ───────────────────
  const selectedElection = elections.find(e => e.id === selectedResultElectionId) || elections[0];
  const selectedCandidates = selectedElection ? candidates.filter(c => c.election_id === selectedElection.id) : [];
  const totalResultVotes = selectedCandidates.reduce((s, c) => s + (c.vote_count || 0), 0);
  const maxResultVotes = selectedCandidates.length > 0 ? Math.max(...selectedCandidates.map(c => c.vote_count || 0)) : 0;
  const topResultCandidates = selectedCandidates.filter(c => (c.vote_count || 0) === maxResultVotes && maxResultVotes > 0);
  const winnerCandidate = topResultCandidates.length === 1 ? topResultCandidates[0] : null;
  const winnerPercentage = winnerCandidate && totalResultVotes > 0 ? Math.round((winnerCandidate.vote_count / totalResultVotes) * 100) : 0;

  // Filtered data (Strictly exclude ADM / Administrator accounts from voter directory)
  ...
  const handleDeleteAllVoters = async () => {
    if (!window.confirm("⚠️ Are you sure you want to permanently delete ALL registered voters from the database? This action is irreversible.")) return;
    try {
      const res = await fetch('/api/admin/voters/all', { method: 'DELETE' });
      const data = await res.json();
      if (res.ok && data.success) {
        showAlert(data.message, 'success');
        fetchAdminStats();
      } else {
        showAlert(data.message || 'Failed to delete voters.');
      }
    } catch (err) {
      showAlert('Error connecting to server.');
    }
  ...
  const handleDeleteAllElections = async () => {
    if (!window.confirm("⚠️ Are you sure you want to permanently delete ALL elections and candidate nominations from the database? This action is irreversible.")) return;
    try {
      const res = await fetch('/api/admin/elections/all', { method: 'DELETE' });
      const data = await res.json();
      if (res.ok && data.success) {
        showAlert(data.message, 'success');
        fetchAdminStats();
      } else {
        showAlert(data.message || 'Failed to delete elections.');
      }
    } catch (err) {
      showAlert('Error connecting to server.');
    }
  ...
  const handleResetDatabase = async () => {
    try {
      ...
      if (res.ok && data.success) {
        ...
        showAlert('✅ Factory reset complete. All data permanently wiped from database.', 'success');
        fetchAdminStats();
      } else {
        showAlert(data.message || 'Failed to reset database.');
      }
    } catch (err) {
      showAlert('Error connecting to server.');
    }
  ...
  // ─── JSX ──────────────────────────────────────────────────────────────────
  if (!adminUser) {
    return <AdminLogin onLogin={handleAdminLogin} alertMsg={alertMsg} navigateTo={navigateTo} />;
  }
    ...
    <div className="main-container">
      ...
      {activeTab === 'overview' && (
        <div style={{ display: 'grid', gap: '1.5rem', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))' }}>
          ...
          {/* Danger Zone: Permanent Database Factory Reset */}
          <div style={{
            gridColumn: '1 / -1',
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                ...
                <span style={{ fontWeight: 800, color: '#ef4444', fontSize: '1rem' }}>Permanent Database Purge (Factory Reset)</span>
              </div>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem', margin: 0, maxWidth: '650px', lineHeight: 1.4 }}>
                Permanently wipes all election polls, candidate nominations, voter registrations, cast ballots, and verification tokens from the database (<code>data.json</code> & MongoDB). Preserves only the primary administrator (ADM-9999).
              </p>
            </div>
            <button
              className="btn"
            ...
            >
              🗑️ Permanently Delete All System Data
            </button>
          </div>
        </div>
      )}
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
              {voters.length > 0 && (
                <button
                  ...
                  style={{ color: '#ef4444', borderColor: 'rgba(239,68,68,0.4)', fontSize: '0.82rem' }}
                >
                  🗑️ Delete All Voters
                </button>
              )}
            ...
            <table className="table">
              ...
              <tbody>
                {filteredVoters.length === 0 && !voterSearch ? (
                  <tr><td colSpan={9} style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '2rem' }}>No voters registered yet.</td></tr>
                ) : filteredVoters.length === 0 ? (
                  <tr><td colSpan={9} style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '2rem' }}>No results for "{voterSearch}"</td></tr>
                ) : (
                  filteredVoters.map((v, i) => (
                    <tr key={i}>
                      ...
                      <td>
                        {v.is_verified === 0 ? (
                          <span style={{ padding: '2px 8px', borderRadius: '8px', fontSize: '0.72rem', fontWeight: 800, background: 'rgba(234, 179, 8, 0.15)', color: '#eab308', border: '1px solid rgba(234, 179, 8, 0.3)' }}>
                            ⏳ Pending OTP
                          </span>
                        ) : (
                          <span style={{ padding: '2px 8px', borderRadius: '8px', fontSize: '0.72rem', fontWeight: 800, background: 'rgba(16, 185, 129, 0.15)', color: '#10b981', border: '1px solid rgba(16, 185, 129, 0.3)' }}>
                            ✓ Verified
                          </span>
                        )}
                      </td>
                      <td><span style={{ padding: '2px 8px', borderRadius: '8px', fontSize: '0.72rem', fontWeight: 800, background: 'rgba(99,102,241,0.12)', color: 'var(--primary)', border: '1px solid rgba(99,102,241,0.25)' }}>{(v.role || 'VOTER').toUpperCase()}</span></td>
                      <td>
                        <button
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '0.78rem', color: '#34d399', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px' }}>
                        Automatic Election Winner
                      </div>
                      <h3 style={{ fontSize: '1.4rem', fontWeight: 900, margin: '2px 0 0 0' }}>
                        {winnerCandidate.name} <span style={{ fontSize: '0.9rem', color: 'var(--text-muted)', fontWeight: 600 }}>({winnerCandidate.party || winnerCandidate.department})</span>
                      </h3>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '1.5rem', fontWeight: 900, color: '#34d399' }}>{winnerCandidate.vote_count || 0} Votes</div>
                    <div style={{ fontSize: '0.82rem', color: 'var(--text-muted)', fontWeight: 700 }}>{winnerPercentage}% Victory Share</div>
                  </div>
                </div>
              ) : topResultCandidates.length > 1 ? (
                <div style={{
                  background: 'rgba(245, 158, 11, 0.12)', border: '1.5px solid var(--accent-gold)',
                  borderRadius: '18px', padding: '1rem 1.5rem', color: '#f59e0b', fontWeight: 800
                }}>
                  ⚠️ Current Standings: TIE between {topResultCandidates.map(c => c.name).join(' & ')} with {maxResultVotes} votes each!
                </div>
              ) : null}
      ...
      {showCreateElectionModal && (
        <div className="modal-backdrop" onClick={e => { if (e.target === e.currentTarget) setShowCreateElectionModal(false); }}>
          <div className="modal-content">
            ...
            <form onSubmit={handleCreateElection} onKeyDown={handleEnterKeyNavigation}>
      ...
      {/* ══════════════════════════════════════════════════════════════════
          MODAL: RESET ALL DATABASE DATA CONFIRM
      ══════════════════════════════════════════════════════════════════ */}
      {showResetConfirm && (
        <div className="modal-backdrop" onClick={e => { if (e.target === e.currentTarget) setShowResetConfirm(false); }}>
          <div className="modal-content" style={{ maxWidth: '460px', border: '1px solid rgba(239, 68, 68, 0.4)', position: 'relative' }}>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '-6px' }}>
              <button
                type="button"
            ...
            <div style={{ textAlign: 'center', padding: '0.5rem 0 1.5rem' }}>
              ...
              <h2 style={{ fontSize: '1.3rem', fontWeight: 900, color: '#ef4444', marginBottom: '0.5rem' }}>
                Factory Reset Database?
              </h2>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: 1.6 }}>
                This will permanently delete <strong>ALL elections, candidates, registered voters, votes, and verification tokens</strong> from both <code>data.json</code> and MongoDB.
              </p>
              <div style={{
                background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.25)',
                borderRadius: '12px', padding: '0.75rem', marginTop: '1rem', fontSize: '0.8rem', color: '#f87171'
              }}>
                🔒 The primary administrator account (<code>ADM-9999</code>) will be preserved for future access. All other data is permanently destroyed.
              </div>
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button
                className="btn"
                style={{ flex: 1.5, background: 'linear-gradient(135deg,#dc2626,#991b1b)', color: '#fff', fontWeight: 800, boxShadow: '0 4px 15px rgba(220,38,38,0.45)' }}
                onClick={handleResetDatabase}
              >
                🗑️ Yes, Purge All Data
              </button>
            </div>
          </div>
        </div>
